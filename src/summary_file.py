...
class SummaryFile:
    ...
    def update_from_answers(self, summaryfile_session: pd.DataFrame):
        # update good answers
        idx_good_answers = summaryfile_session[
            summaryfile_session["answers"]==summaryfile_session["result"]
        ].index
        self.df.loc[idx_good_answers, "success"] =+ 1
        logger.debug(f"idx_good_answers={idx_good_answers}")
        # update bad answers
        idx_bad_answers = summaryfile_session[
            summaryfile_session["answers"]!=summaryfile_session["result"]
            ].index
        self.df.loc[idx_bad_answers, "failure"] =+ 1
        logger.debug(f"idx_bad_answers={idx_bad_answers}")

        assert len(idx_good_answers) + len(idx_bad_answers) == len(summaryfile_session)

        self._calculate_weights_from_success_failure()
        logger.debug(f"self.df.loc[summaryfile_session.index]=\n{self.df.loc[summaryfile_session.index]}")
